...
qwen_vl_utils.vision_process.MIN_PIXELS = 32*32
qwen_vl_utils.vision_process.MAX_PIXELS = 768*28*28
# ......
...
class Qwen2VLPlugin(BasePlugin):
    ...
    def _regularize_videos(self, videos: Sequence["VideoInput"], **kwargs) -> List[List["ImageObject"]]:
        ...
        for video in videos:
            ele = {"video": video}
            sampled_video, sampled_fps = qwen_vl_utils.fetch_video(ele, return_video_sample_fps=True)
            results.append(sampled_video)
            fps_per_video.append(sampled_fps)
    ...
    def _get_mm_inputs(
        ...
        if len(images) != 0:
            images = self._regularize_images(
                images,
                image_max_pixels=getattr(processor, "image_max_pixels", 768 * 28* 28),
                image_min_pixels=getattr(processor, "image_min_pixels", 32 * 32),
            )["images"]
